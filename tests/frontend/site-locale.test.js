...
test('late hydration updates and changed text on reused nodes are translated',async()=>{
 ...
 vm.runInContext("translatePage('zh')",context);
 assert.equal(document.querySelector('p').textContent,'你的密钥始终属于你');
 document.querySelector('p').firstChild.nodeValue='Five deterministic gates';
 vm.runInContext("translatePage('zh')",context);assert.equal(document.querySelector('p').textContent,'五个确定性关卡');
 document.querySelector('p').innerHTML='<span>Your agent sees a </span><b>policy</b><span> result, never your private thresholds.</span>';
 vm.runInContext("translatePage('zh')",context);assert.equal(document.querySelector('p').textContent,'代理只能看到策略结果，永远不会看到你的私有阈值。');
});
