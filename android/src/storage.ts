...
type Envelope = { v: 1; address: string; salt: string; box: Box };
...
export async function createWallet(mnemonic: string, password: string) {
  ...
  if (password.length < 10) throw new Error("Use at least 10 characters. / 密码至少需要10个字符。");
  const version = epoch;
...
export async function unlock(password: string | null) {
  ...
  try {
    ...
    } else key = await passwordKey(password, hexToBytes(saved.salt));
